...
const FavouriteScreen = ({ history, location }) => {
	...
	return (
		...
		>
			{error ? <Alert type='errpr' message='Oops! Something went wrong.' /> : null}
			{loading ? (
				<Loading />
			) : (
				<DIV>
					<h2 style={{ color: 'white' }}>Favourite Contacts</h2>
					<h2 style={{ color: 'white' }}>{list.length === 0 ? 'Empty List' : null}</h2>

					{list.map((contact) => {
						return (
							<PlaceHolder onClick={() => handleDetails(contact.id)} key={contact.id}>
